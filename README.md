...
| 聞き流し | 連続再生。速度0.8〜2.0倍、再生モード(中→日→中 / 中のみ / 3回 / 高速 / **シャドーイング**) |
| 瞬発練習 | 日本語→中国語の即答練習。制限時間後に答え+音声+SRS評価+発音チェック |
| 発音練習 | お手本を聞いて発音→音声認識で判定(声調違いは橙色で表示) |
| 四声クイズ | 単語を聞いて指定の字の声調を当てる(多音字は文脈読みで判定) |
| 長文 | 段落・文単位の表示と再生 |
...
「教材パック」画面の **内蔵教材ライブラリ** から、2,800件超の教材(化学・材料/貿易・物流/営業・商談/価格交渉・納期・品質/電話対応/ウェブ会議/工場査察/ホテル/日常会話/口語表現/新聞・時事/半導体/化学合成/有機化学/中国・台湾の歴史・地理・社会/長文など24パック)をワンクリックで追加できます。「すべて追加」でまとめて追加も可能です。追加済みの教材は重複としてスキップされるので、何度押しても安全です。ライブラリが更新された後も、再度「すべて追加」を押せば新規分だけが取り込まれます。
...
- **✨ おまかせ学習**: ホームの「おまかせ学習(30件)」を押すと、「今日のSRS復習 → 苦手 → 未学習 → その他」を自動ミックスした30件が聞き流しに流れます。聞き流し・瞬発・発音練習・四声クイズの対象セレクターからも「✨ おまかせ30件」を選べます
- **画面オフ対策**: 再生中は画面の自動消灯を防止し(Wake Lock)、ロック画面やイヤホンのボタンから再生/停止/前後の操作ができます(Media Session)。※OS・ブラウザにより挙動は異なります

## シャドーイングと発音練習

### シャドーイング(聞き流し画面の再生モード)

「再生 → 復唱ポーズ(カウントダウン表示)→ 再確認再生」を自動で繰り返します。

- ポーズの長さは文の長さから自動計算(短め/標準/長めを選択可)
- 「🙈 文字を隠す」をONにすると、耳だけで復唱する上級モードになります

### 発音練習(🎤)

お手本を聞いて発音すると、ブラウザの音声認識で判定します。

- 判定はピンイン音節ベース: **緑=OK / 橙=声調ちがい / 赤=音がちがう**
- 瞬発練習の回答画面にも「🎤 発音チェック」があります
- **動作条件**: Chrome / Edge(PC・Android)、HTTPS(公開サイトやlocalhost)、インターネット接続、マイク許可
- LAN経由(`http://192.168.x.x:5173`)ではマイクが使えないため、スマホでは公開サイト側をご利用ください
- 音声認識は文脈で補正するため、判定は「目安」です。声に出す回数を重視してください
...
教材・お気に入り・苦手・SRS状態・学習履歴を、非公開のGitHub Gist経由で複数端末間で同期できます。

### 初回設定(両端末で同じトークンを貼るだけ)

1. [トークン作成ページ](https://github.com/settings/tokens/new?scopes=gist&description=chinese-trainer-sync)でGitHubトークンを作成(**gist権限のみ**)
2. 設定画面 →「☁ 端末間同期」にトークンを貼り付け → 接続テスト → 今すぐ同期
3. もう一方の端末でも同じトークンを貼って同期(同期先Gistは自動で見つかります)

### 仕組みと注意

- 同期は双方向マージです。同じ教材は**更新が新しい方**が残ります
- 「起動時に自動同期」ON(既定)なら、アプリを開くたびに自動で同期します。ホーム画面の「☁ 同期」でも手動実行できます
- 設定と学習キューは端末ごとに独立(同期対象外)です
- **削除は同期されません**(片方で削除しても、もう片方に残っていれば次の同期で復活します)
- トークンは各端末のブラウザ内にのみ保存され、同期データやバックアップJSONには含まれません
